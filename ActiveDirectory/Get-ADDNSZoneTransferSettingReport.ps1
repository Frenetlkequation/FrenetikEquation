<#
.SYNOPSIS
Reports on AD Zone transfer settings.

.DESCRIPTION
This script gathers AD Zone transfer settings data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDNSZoneTransferSettingReport.ps1
...
Write-Host "Retrieving DNS Zone transfer settings report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($zone in $zones) {
        [PSCustomObject]@{
            ReportType      = 'Zone transfer settings'
