<#
.SYNOPSIS
Reports on AD Selective authentication.

.DESCRIPTION
This script gathers AD Selective authentication data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADTrustSelectiveAuthenticationReport.ps1
...
Write-Host "Retrieving AD trust Selective authentication report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($trust in $trusts) {
        [PSCustomObject]@{
            ReportType              = 'Selective authentication'
            Name                    = $trust.Name
