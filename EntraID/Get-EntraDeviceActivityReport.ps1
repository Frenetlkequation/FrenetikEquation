<#
.SYNOPSIS
Reports on Entra ID devices in a activity view.

.DESCRIPTION
This script queries Microsoft Graph for devices and presents a activity report for governance, security, and operations review.

.EXAMPLE
.\Get-EntraDeviceActivityReport.ps1
...
$report = foreach ($item in $items | Sort-Object -Property approximateLastSignInDateTime -Descending | Select-Object -First $Top) {
    [PSCustomObject]@{
        DisplayName = $item.displayName
        OperatingSystem = $item.operatingSystem
        TrustType = $item.trustType
        AccountEnabled = $item.accountEnabled
        LastUpdatedValue = $item.approximateLastSignInDateTime
    }
}
