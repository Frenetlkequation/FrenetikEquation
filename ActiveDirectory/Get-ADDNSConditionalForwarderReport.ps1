<#
.SYNOPSIS
Reports on AD Conditional forwarders.

.DESCRIPTION
This script gathers AD Conditional forwarders data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDNSConditionalForwarderReport.ps1
...
Write-Host "Retrieving DNS Conditional forwarders report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($zone in $zones) {
        [PSCustomObject]@{
            ReportType      = 'Conditional forwarders'
