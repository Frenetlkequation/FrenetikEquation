<#
.SYNOPSIS
Reports on AD Duplicate IP records.

.DESCRIPTION
This script gathers AD Duplicate IP records data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDNSDuplicateIpRecordReport.ps1
...
Write-Host "Retrieving DNS Duplicate IP records report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($zone in $zones) {
        [PSCustomObject]@{
            ReportType      = 'Duplicate IP records'
