<#
.SYNOPSIS
Reports on AD SMB protocol configuration.

.DESCRIPTION
This script gathers AD SMB protocol configuration data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDCSmbProtocolConfigurationReport.ps1
...
Write-Host "Retrieving domain controller SMB protocol configuration report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($dc in $domainControllers) {
        ...
        [PSCustomObject]@{
            ReportType      = 'SMB protocol configuration'
