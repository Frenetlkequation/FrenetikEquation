<#
.SYNOPSIS
Reports on AD LDAP binding security.

.DESCRIPTION
This script gathers AD LDAP binding security data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDCLdapBindingSecurityReport.ps1
...
Write-Host "Retrieving domain controller LDAP binding security report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($dc in $domainControllers) {
        ...
        [PSCustomObject]@{
            ReportType      = 'LDAP binding security'
