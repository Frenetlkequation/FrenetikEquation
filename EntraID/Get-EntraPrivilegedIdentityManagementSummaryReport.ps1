<#
.SYNOPSIS
Reports on Entra ID privileged role schedule instances in a summary view.

.DESCRIPTION
This script queries Microsoft Graph for privileged role schedule instances and presents a summary report for governance, security, and operations review.

.EXAMPLE
.\Get-EntraPrivilegedIdentityManagementSummaryReport.ps1
...
$report = if ($items.Count -eq 0) {
    ...
    $items | Group-Object -Property memberType | ForEach-Object {
        [PSCustomObject]@{
            memberType = if ($_.Name) { $_.Name } else { 'Unspecified' }
            Count = $_.Count
        }
    }
}
...
if ($report.Count -eq 0) {
    ...
    Write-Host "Retrieved $($report.Count) grouped record(s) for privileged role schedule instances." -ForegroundColor Green
