<#
.SYNOPSIS
Reports on Active Directory domain controller health.

.DESCRIPTION
This script retrieves domain controller information and performs basic
connectivity checks to help identify unreachable or unhealthy controllers.
...
Write-Host "Retrieving domain controller health information..." -ForegroundColor Cyan
...
if ($report.Count -eq 0) {
    ...
    $healthy = ($report | Where-Object { $_.Status -eq "Healthy" }).Count
    $unreachable = ($report | Where-Object { $_.Status -eq "Unreachable" }).Count

    Write-Host "Retrieved $($report.Count) domain controller(s)." -ForegroundColor Green
    Write-Host "  Healthy: $healthy" -ForegroundColor Yellow
