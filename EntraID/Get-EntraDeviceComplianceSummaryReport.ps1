<#
.SYNOPSIS
Reports on Microsoft Entra device compliance summary.

.DESCRIPTION
This script retrieves Intune managed devices and summarizes compliance state,
operating system, and last check-in activity for device health review.
...
$report = Get-GraphCollection -Uri "https://graph.microsoft.com/v1.0/deviceManagement/managedDevices?`$select=id,deviceName,operatingSystem,complianceState,managementAgent,lastSyncDateTime,userPrincipalName" |
    ForEach-Object {
        [PSCustomObject]@{
...
if ($report.Count -eq 0) {
    ...
    Write-Host "Retrieved $($report.Count) managed device(s). Non-compliant: $nonCompliant" -ForegroundColor Green
