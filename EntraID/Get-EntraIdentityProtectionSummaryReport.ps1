<#
.SYNOPSIS
Reports on Entra ID risky users in a summary view.

.DESCRIPTION
This script queries Microsoft Graph for risky users and presents a summary report for governance, security, and operations review.

.EXAMPLE
.\Get-EntraIdentityProtectionSummaryReport.ps1
...
$report = if ($items.Count -eq 0) {
    ...
    $items | Group-Object -Property riskLevel | ForEach-Object {
        [PSCustomObject]@{
            RiskLevel = if ($_.Name) { $_.Name } else { 'Unspecified' }
            Count = $_.Count
        }
    }
}
...
if ($report.Count -eq 0) {
    ...
    Write-Host "Retrieved $($report.Count) grouped record(s) for risky users." -ForegroundColor Green
