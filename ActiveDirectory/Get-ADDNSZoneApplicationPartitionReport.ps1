<#
.SYNOPSIS
Reports on AD Zone application partitions.

.DESCRIPTION
This script gathers AD Zone application partitions data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDNSZoneApplicationPartitionReport.ps1
...
Write-Host "Retrieving DNS Zone application partitions report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($zone in $zones) {
        [PSCustomObject]@{
            ReportType      = 'Zone application partitions'
