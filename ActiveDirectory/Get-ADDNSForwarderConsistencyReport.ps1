<#
.SYNOPSIS
Reports on AD Forwarder consistency.

.DESCRIPTION
This script gathers AD Forwarder consistency data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDNSForwarderConsistencyReport.ps1
...
Write-Host "Retrieving DNS Forwarder consistency report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($zone in $zones) {
        [PSCustomObject]@{
            ReportType      = 'Forwarder consistency'
