<#
.SYNOPSIS
Reports on Entra ID privileged role schedule instances in a activity view.

.DESCRIPTION
This script queries Microsoft Graph for privileged role schedule instances and presents a activity report for governance, security, and operations review.

.EXAMPLE
.\Get-EntraPrivilegedIdentityManagementActivityReport.ps1
...
$report = foreach ($item in $items | Sort-Object -Property startDateTime -Descending | Select-Object -First $Top) {
    [PSCustomObject]@{
        PrincipalId = $item.principalId
        RoleDefinitionId = $item.roleDefinitionId
        StartDateTime = $item.startDateTime
        EndDateTime = $item.endDateTime
        LastUpdatedValue = $item.startDateTime
    }
}
