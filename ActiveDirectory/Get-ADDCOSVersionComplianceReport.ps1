<#
.SYNOPSIS
Reports on AD OS version compliance.

.DESCRIPTION
This script gathers AD OS version compliance data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDCOSVersionComplianceReport.ps1
...
Write-Host "Retrieving domain controller OS version compliance report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($dc in $domainControllers) {
        ...
        [PSCustomObject]@{
            ReportType      = 'OS version compliance'
