<#
.SYNOPSIS
Reports on Entra ID devices in a summary view.

.DESCRIPTION
This script queries Microsoft Graph for devices and presents a summary report for governance, security, and operations review.

.EXAMPLE
.\Get-EntraDeviceSummaryReport.ps1
...
$report = if ($items.Count -eq 0) {
    ...
    $items | Group-Object -Property trustType | ForEach-Object {
        [PSCustomObject]@{
            TrustType = if ($_.Name) { $_.Name } else { 'Unspecified' }
            Count = $_.Count
        }
    }
}
...
if ($report.Count -eq 0) {
    ...
    Write-Host "Retrieved $($report.Count) grouped record(s) for devices." -ForegroundColor Green
