<#
.SYNOPSIS
Reports on AD LDAP channel binding.

.DESCRIPTION
This script gathers AD LDAP channel binding data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDCLdapChannelBindingReport.ps1
...
Write-Host "Retrieving domain controller LDAP channel binding report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($dc in $domainControllers) {
        ...
        [PSCustomObject]@{
            ReportType      = 'LDAP channel binding'
