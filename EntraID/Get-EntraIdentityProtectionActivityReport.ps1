<#
.SYNOPSIS
Reports on Entra ID risky users in a activity view.

.DESCRIPTION
This script queries Microsoft Graph for risky users and presents a activity report for governance, security, and operations review.

.EXAMPLE
.\Get-EntraIdentityProtectionActivityReport.ps1
...
$report = foreach ($item in $items | Sort-Object -Property riskLastUpdatedDateTime -Descending | Select-Object -First $Top) {
    [PSCustomObject]@{
        UserPrincipalName = $item.userPrincipalName
        RiskLevel = $item.riskLevel
        RiskState = $item.riskState
        RiskLastUpdatedDateTime = $item.riskLastUpdatedDateTime
        LastUpdatedValue = $item.riskLastUpdatedDateTime
    }
}
