<#
.SYNOPSIS
Reports on AD Duplicate host records.

.DESCRIPTION
This script gathers AD Duplicate host records data and formats a PSCustomObject report for quick review.

.EXAMPLE
.\Get-ADDNSDuplicateHostRecordReport.ps1
...
Write-Host "Retrieving DNS Duplicate host records report..." -ForegroundColor Cyan
...
try {
    ...
    $report = foreach ($zone in $zones) {
        [PSCustomObject]@{
            ReportType      = 'Duplicate host records'
